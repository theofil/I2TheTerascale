...
float xMin      = 0;
float xMax      = 500;
string plotName = "mqqb"; // don't put special symbols and spaces here
string ylabel   = "events / bin";
string xlabel   = "m(q,q,b) [GeV]";
bool setLogy    = false; // log scale for y-axis
...
{    
	...
	for(Long64_t j = 0 ; j<nEvents ; ++j)
	{
	    events[i]->GetEntry(j); 
            ...
            {
                float muPt = 0;
                TLorentzVector muon(Muon_Px[0], Muon_Py[0], Muon_Pz[0], Muon_E[0]); 
                muPt = muon.Pt();
                int NBJet = 0;
                ...
                for(int jetIndex = 0; jetIndex < NJet; jetIndex++) 
                {
                    TLorentzVector myJet(Jet_Px[jetIndex], Jet_Py[jetIndex], Jet_Pz[jetIndex], Jet_E[jetIndex]);

                    if(Jet_btag[jetIndex] > 1.74) 
                    {
                        if(NBJet==0) {b1 = myJet;}
                        if(NBJet==1) {b2 = myJet;}
                        NBJet += 1; 
            ...
            if(selection) 
            {
                histo[i]->Fill(var1, EventWeight);
                histo[i]->Fill(var2, EventWeight);
            }
