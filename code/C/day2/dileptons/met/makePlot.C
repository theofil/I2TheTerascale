...
float xMin      = 0;
float xMax      = 200;
string plotName = "met"; // don't put special symbols and spaces here
string ylabel   = "events / bin";
string xlabel   = "MET [GeV]";
...
{    
	...
	for(Long64_t j = 0 ; j<nEvents ; ++j)
	{
	    events[i]->GetEntry(j); 
            ...
            if(triggerIsoMu24 && NMuon >= 2) 
            if(sqrt(Muon_Px[0]*Muon_Px[0] + Muon_Py[0]*Muon_Py[0])> 25 )
            {
                selection = true; 
                var       = sqrt(MET_px*MET_px + MET_py*MET_py) ;
            }
